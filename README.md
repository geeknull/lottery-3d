...
- **双屏遥控**（🖵 按钮）：点开「控制窗」拖到笔记本屏，大屏只管投影展示、笔记本上选奖项/开始停/看中奖名单；控制窗连上后展示窗自动隐藏操作按钮，投影画面更干净。需同一台电脑同一浏览器、Safari 15.4+ 或现代 Chrome/Edge/Firefox
...
- **核心抽奖**（3D 球、配置、名单、开始/停、作废/撤销、进度保存）在 **2020 年后的任何主流浏览器**都能用。
- **全部功能完整**需要 **Chrome / Edge 92+、Firefox 95+、Safari 15.4+**，并用 **HTTPS 或 localhost** 访问。
- 老浏览器、`file://` 双击打开、内网 HTTP 部署时，**核心抽奖照常工作**，仅部分增强功能降级（如加密公平承诺退回弱哈希、双屏不可用），且启动会温和提示、不阻断使用。
- 关键 Web API（`crypto.subtle`、`crypto.randomUUID`、`BroadcastChannel`）都有降级处理，不会因不支持而崩溃。
